...
def distribute_domain(domain_size, num_procs):
    subdomain_size = domain_size // num_procs
    remainder = domain_size % num_procs
    subdomains = [subdomain_size + 1 if i < remainder else subdomain_size for i in range(num_procs)]
    offsets = [sum(subdomains[:i]) for i in range(num_procs)]
    return subdomains, offsets


subdomains, offsets = distribute_domain(N, size)
...
if rank == 0:
    ...
    test = hf.split(Wmat, subdomains, rank)
